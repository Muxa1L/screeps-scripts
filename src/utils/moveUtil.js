...
function moveCreep(creep, target, opts) {
    ...
    const moveOpts = {
        ...
        // NOTE: do NOT pass serialize:true here. On this engine version
        // moveTo throws "path is not an array" every tick with that option
        // (verified live), silently freezing the creep — the error propagates
        // up to safeRun and _lastMoveResult never gets recorded.
    };
    ...
    if (mvr === ERR_NO_PATH) {
        ...
        const retryOpts = { reusePath: 2, maxOps: 1500, ignoreCreeps: true };
